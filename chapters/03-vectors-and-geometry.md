...
<div class="chapter-intro" markdown="1">
  ...
  <p>这一章会把“单个数字”推进到“由<strong>多个特征组成的对象</strong>”，让你开始用<strong>向量</strong>来理解样本、距离和相似性，并为下一章把多个对象进一步统一组织成矩阵做好准备。</p>
</div>
...
向量（vector）可以先理解为一个按顺序排列的数值列表。这里最重要的不是一开始追求抽象定义，而是先建立“整体对象”的意识：向量不是若干数字随便放在一起，而是若干有固定顺序、共同描述同一个对象的数字。
...
在二维里，同一个向量还可以同时用“点”和“箭头”两种方式来看：

![向量作为点和箭头](../assets/figures/ch03_point_and_arrow.png)

先看右上方的坐标点 \((3,2)\)，它表示这个对象在两个维度上的取值分别是多少；再看从原点指向这个点的箭头，就会发现同一个向量也可以被理解成“从原点出发走到 \((3,2)\) 的位移”。也就是说，点视角强调的是“这个对象落在什么位置”，箭头视角强调的是“它朝什么方向走了多远”。后面机器学习里这两种看法会反复切换，所以这里越早把这层双重视角看顺越好。
...
如果把两个点直接画出来，距离的几何意义会更直观：

![两点距离图示](../assets/figures/ch03_distance.png)

图中的点 `A` 和点 `B` 可以理解成两条样本记录，或者两个由二维特征描述的对象。把 `A` 和 `B` 直接连起来的那条线段，表示的正是“从一个点走到另一个点最直接要走多远”。这样再回头看距离公式，就更容易接受它不是凭空冒出来的新规则，而是在把图中的横向差、纵向差和总长度写成可计算的形式。
...
如果想把这层几何感觉再往前推进一步，可以这样想：当两个向量大致朝同一个方向时，它们对应分量往往会同时偏正或同时偏大，于是乘起来再相加，结果通常更大；如果两个向量方向明显不一致，有些位置上的分量就会互相“打架”，于是总和会变小。也就是说，点积并不是只在机械地算一个数，它也在粗略反映“两个方向是不是在往同一边使劲”。
...
如果把这个过程想成“老师给每个特征分别打分再汇总”，就更容易读顺了。这里的 \(w\) 并不是在凭空参与计算，而是在回答“每个特征在最终结果里该占多大权重”。因此，点积的作用不是单纯把两个向量并排摆放，而是把“特征值”和“对应权重”一一配对，再压缩成一个总分。
...
这告诉我们：两个对象是否相似，不仅和“大小差多少”有关，也和“方向是否一致”有关。向量 \((1,1)\) 和 \((10,10)\) 虽然长度差很多，但它们都在朝“右上方”这个同一方向延伸，所以更像是“规模不同、结构相似”；而 \((1,1)\) 和 \((1,-1)\) 虽然长度相近，却一个朝右上、一个朝右下，说明它们内部各分量的变化关系已经明显不同。前者更多反映量级差异，后者则更像是在问“结构是否相像”。这也是为什么后面很多模型既关心距离，也关心方向关系。
...
在此基础上，长度、距离和点积又进一步把“整体表示”推进为“整体计算”。长度和距离帮助我们判断对象有多大、彼此有多近，点积则让多个特征的共同作用可以被压缩成一个统一的加权结果。对第一次学习的人来说，本章真正需要带走的，不只是向量加法或数乘的计算规则，而是一个稳定的核心直觉：机器学习中的一条样本、一个特征向量、一次线性打分，本质上都建立在向量这种整体表示方式之上。也正因为如此，下一章才会继续追问：如果有很多条样本、很多组参数、很多次统一变换，这些整体对象又该怎样被更高效地组织起来。
