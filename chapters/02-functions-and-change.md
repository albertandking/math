...
但从教材学习的角度看，更重要的理解是：函数描述的是一条从输入到输出的规则。
...
在数学里，函数正是把这种“规则关系”用更精确、更可分析的方式表达出来。
...
读者也许知道可以代入数值进行计算，但未必能够立刻把握它所描述的变化特征。
...
这实际上已经非常接近导数所要回答的问题，只是表述还停留在较直观的层面。
