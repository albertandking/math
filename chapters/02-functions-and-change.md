...
如果说第 1 章主要解决的是“怎么看公式”，那么这一章进一步要解决的是“公式究竟在描述什么变化”。对机器学习来说，这一步非常关键，因为无论是模型预测、参数调整还是损失下降，本质上都离不开“变化关系”的理解。只有先学会从变化的角度看函数，后面读导数、梯度和优化时才不会觉得它们像突然出现的新语言。

建议读者先把“函数”理解成一条规则，而不是一段代数操作；再通过图像、表格和例子观察同一个规则如何以不同形式出现。阅读时不必急于把每一个变化率公式都算到非常熟练，更重要的是抓住这样一个核心问题：当输入改变一点时，输出究竟是怎样跟着变的。如果这层理解建立起来了，后面进入微积分会顺畅很多。
