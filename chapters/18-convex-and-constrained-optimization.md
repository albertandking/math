...
![无约束最优点被边界截住图示](../assets/figures/ch18_boundary_optimum_shift.svg)

先看这张图时，最值得先盯住的是左边抛物线最低点和右边竖直边界线之间的关系。抛物线自己的“心愿”是把解放在 \(x=-1\) 的谷底，但约束 \(x \ge 0\) 把左半边整块裁掉了，于是优化过程就只能在允许区域里重新找最低点，最后停在边界 \(x=0\)。这样一来，约束为什么会把最优点“推到边界上”就不再只是口头说法，而会变成一张一眼能读出“原本想去哪里、后来又被拦在了哪里”的图。
...
这一节先看“地形本身”和“允许区域”怎样一起决定解，再看凸性为什么会让局部判断更值得信任。读图时建议按“先把第 1、2 步读成问题结构长什么样，再把第 3、4 步读成最优解最终会落在哪里”的顺序推进。只要读完后你能回答“凸目标和凸可行域为什么会改变我们对最优解位置的判断”，这一节的图示主线就算真正建立起来。

![凸地形、可行域与边界最优点图示](../assets/figures/ch18_convex_feasible_boundary.svg)

<span class="figure-inline-label figure-inline-label-order">读图顺序</span>可以先只看左边那一半，观察蓝色等高线怎样围成一个标准“碗形”地形，为什么最低点只有一个；接着再把视线移到右边那一半，看同样的碗形地形一旦叠加上橙色可行域后，问题怎样从“整个碗底在哪里”变成“允许区域里最低的位置在哪里”。此时红色虚线连起的两个点最值得观察：原本的无约束最低点可能已经不在可行域里，于是最优解就会被推到边界上。这样一来，约束优化就不再只是多写几条条件，而会变成“目标地形”和“允许区域”共同决定解的位置。
...
<div class="ml-loop">
  <div class="ml-loop-head">
    ...
    <p><span class="figure-inline-label figure-inline-label-order">读图顺序</span>先看目标函数是不是“碗形”，再看允许区域是不是规整，最后再判断最优解落在内部还是边界。</p>
  </div>
...
<span class="figure-inline-label figure-inline-label-takeaway">读后应会</span>可以马上问自己两个问题：**如果地形本身不规整，局部下降方向还值不值得信任？如果允许区域收紧了，最低点会不会被推到边界上？** 只要这两个问题开始稳定，凸优化和约束优化就不会再只是两组定义，而会重新变成可以读图、可以判断的问题结构。
