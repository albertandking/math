...
理解线性模型时，最先要建立的画面，是平面上一组散点和一条拟合直线。读这种图时，先不要急着想公式，而是先看这条直线为什么不是要穿过每一个点，而是要在整体上尽量贴近这些点。再看每个点到直线之间的垂直差距，就会明白误差和损失函数到底在衡量什么：它们衡量的并不是一句抽象的“模型好不好”，而是预测值和真实值之间到底差了多少。

逻辑回归则需要另一幅画面。先看 sigmoid 曲线时，最关键的是理解它在做“压缩”：线性分数本来可以取任意实数，但经过 sigmoid 后，会被稳定地压到 \(0\) 到 \(1\) 之间，于是才有了概率解释。再把这条曲线和“分类边界 + 两类样本点”的图放在一起看，就会更容易明白：逻辑回归前半段仍然是在做线性打分，后半段只是在把这个分数翻译成更适合分类任务的概率语言。
