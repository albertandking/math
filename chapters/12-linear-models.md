...
理解线性模型时，最先要建立的画面，是平面上一组散点和一条拟合直线。读这种图时，先不要急着想公式，而是先看这条直线为什么不是要穿过每一个点，而是要在整体上尽量贴近这些点。再看每个点到直线之间的垂直差距，就会明白误差和损失函数到底在衡量什么：它们衡量的并不是一句抽象的“模型好不好”，而是预测值和真实值之间到底差了多少。若再往前想一步，就会发现这条直线其实是在做一种“整体折中”: 它不可能让所有点都满意，只能尽量让大多数点的误差都不要太离谱。

要把逻辑回归真正看明白，最值得先看的，就是“同一条线性打分规则怎样一边划出分类边界，一边经过 sigmoid 变成概率”。

![逻辑回归边界与 sigmoid 概率图示](../assets/figures/ch12_logistic_boundary.svg)

逻辑回归则需要另一幅画面。看左边这幅图时，先把虚线理解成线性分数 \(w \cdot x + b = 0\) 对应的分类边界: 它把平面分成两侧，一侧的样本更偏向负类，另一侧的样本更偏向正类。此时模型并不是先天知道“红点”“绿点”是什么，而是先根据 \(w \cdot x + b\) 的正负和大小给每个样本打一个线性分数，再由这条边界决定“站在哪一侧”。

再看右边的 sigmoid 曲线时，最关键的是理解它在做“压缩”：线性分数本来可以取任意实数，但经过 sigmoid 后，会被稳定地压到 \(0\) 到 \(1\) 之间，于是才有了概率解释。图中 \(z=0\) 对应的正是 \(p=0.5\)，也就是边界位置；落在边界正侧的样本，其线性分数更大，概率就更接近 \(1\)；落在负侧的样本，线性分数更小，概率就更接近 \(0\)。这样把左边的分类边界和右边的 sigmoid 曲线连起来看，逻辑回归就不再只是“算一个概率”，而会变成“先在线性空间里分边，再把离边界的远近翻译成概率强弱”的完整过程。

读分类边界图时，还可以特别注意边界两侧样本的“离线远近”。离边界越远，通常说明线性分数的绝对值越大，sigmoid 给出的概率也会更接近 \(0\) 或 \(1\)；而靠近边界的点，则更像模型当前仍然拿不太准的样本。如果再配合正则化前后边界变化的对比图去看，就会更容易理解：正则化并不是在凭空加惩罚，而是在防止边界为了迎合训练样本而弯得过于激进、贴得过于死。
