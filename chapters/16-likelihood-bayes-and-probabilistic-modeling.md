...
的项。把所有样本加总后，最大化对数似然就等价于最小化平方误差和。也就是说，线性回归里常见的平方误差，并不是随手指定出来的，而是在“高斯噪声”这个概率假设下自然长出来的。

![高斯噪声下残差大小与似然高低图示](../assets/figures/ch16_gaussian_squared_error.svg)

先看这张图时，最值得先盯住的是钟形曲线中间靠近 \(r=0\) 的高峰位置。它在提醒你：当预测值和真实值很接近时，残差会落在高斯分布最“常见”的区域里，因此这种观测更容易被当前参数解释；反过来，残差一旦跑到两侧尾部，概率密度就会明显降低。这样一来，“平方误差越大，负对数似然代价越重”就不再只是公式推导，而会重新变成一张可以直接看出“偏得越远，解释越吃力”的图。

这个例子特别重要，因为它说明了一个更大的原则：训练目标往往不是孤立设计的，它背后常常对应着某种数据生成假设。顺着这个思路往下读，下一步最自然的问题就是：如果输出不再是连续值，而是某个类别出现的概率，那么负对数似然又会长成什么样？
...
这一节先看“参数、分布、数据、目标函数”之间是谁在解释谁，再看训练为什么会从“参数定义分布”回到“固定数据后比较参数”。读图时建议按“先顺着参数到分布的方向读，再逆着已有数据回头看怎样挑参数”的顺序推进。只要读完后你能回答“什么时候在看概率，什么时候在看似然或后验”，这一节的图示主线就算真正建立起来。

![最大似然与最大后验关系图示](../assets/figures/ch16_likelihood_map_flow.svg)

<span class="figure-inline-label figure-inline-label-order">读图顺序</span>先盯住上面那条从左到右的主线，观察参数 \(\theta\) 怎样先定义一个概率模型，而这个概率模型再去解释已经观察到的数据 \(D\)；再把视线移到下面两块橙色区域，比较最大似然和最大后验怎样共同做“回头挑参数”这件事。只不过最大似然只看 \(P(D\mid\theta)\)，而最大后验还额外接入了右侧紫色的先验 \(P(\theta)\)。这样一来，两者的关系就不会再像两套分开的定义，而会变成同一条流程上的两个不同落点。
...
<div class="ml-loop">
  <div class="ml-loop-head">
    ...
    <p><span class="figure-inline-label figure-inline-label-order">读图顺序</span>先顺着主流程读“参数怎样定义分布”，再逆着想“固定数据后，我们怎样比较不同参数的解释力”。</p>
  </div>
...
<span class="figure-inline-label figure-inline-label-takeaway">读后应会</span>最值得再回头复述的一句是 **参数先决定分布，分布再解释数据，而训练则是在已有观测数据面前反过来比较参数。** 如果你已经能把第 1、2 步读成“参数如何定义一个数据世界”，再把第 3、4 步读成“真实数据出现后怎样回头挑参数”，那么似然和后验就不会再像突兀的新名词，而会重新回到“模型怎样解释观测数据”这条主线上。
