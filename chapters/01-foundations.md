...
<div class="chapter-intro">
  ...
  <p>这一章作为全书的起点，主要任务不是增加计算难度，而是帮助读者建立阅读数学表达的基本能力，使公式、文字、表格和图像之间能够相互连通。</p>
</div>

<div class="reading-focus" markdown="1">
<strong>阅读重点</strong>

- 把字母理解为“量的名称”，先建立符号阅读能力
- 把公式、表格和图像看作同一关系的不同表达形式
...
读这张图时，首先应当注意点 `(0, 1)`，它说明当 \(x = 0\) 时，\(y = 1\)；再看点 `(1, 3)` 和 `(2, 5)`，它们正好对应前面函数值表中的结果。更关键的是，这些点并不是零散分布的，而是整齐地落在同一条直线上，这就说明这里描述的是一种稳定的线性变化。只要读者能够把“表格中的数字变化”和“图上的直线变化”对应起来，后面学习函数图像时就已经迈出了最关键的一步。
...
本章在视觉化上尤其重要，因为它承担的是“帮助读者开始读懂数学”的任务。后续如果继续完善配图，建议至少补充三类图示：第一类是变量与常量示意图，用一个简单表达式明确标出哪些量能变、哪些量暂时不变；第二类是函数值表到图像的转化图，让读者看到表格中的每一组数是怎样落到坐标系中的；第三类是一次函数 \( y = 2x + 1 \) 的直线图，在图上进一步标清截距和“每向右 1 格、向上 2 格”的变化方式。如果后续使用 draw.io 继续细化图示，本章的图最好统一采用“坐标图 + 标注框 + 简短说明”的形式，使视觉风格保持稳定。
...
??? note "参考答案"
    1. 变量是 \(x\) 和 \(y\)；常量是 \(4\) 和 \(3\)。
    2. 代入 \(y = 3x + 2\)：\(x = 0 \to 2\)，\(x = 1 \to 5\)，\(x = 2 \to 8\)，\(x = 3 \to 11\)。
    3. \(y = 5x + 10\) 表示：每当输入 \(x\) 增加 1，输出 \(y\) 就增加 5；当 \(x = 0\) 时仍有一个基础值 10。
    4. \(f(x) = x + 7\)：\(f(1) = 8\)，\(f(5) = 12\)，\(f(10) = 17\)。
    5. 开放题，答案不唯一。例如：每月话费 = 0.1 × 通话分钟数 + 20，写成 \(y = 0.1x + 20\)，其中 \(x\) 是通话分钟数，\(y\) 是话费。
